...
updated_at: 2026-07-09T17:34:29Z
---
...
- browserid.me + mingo.place both live on v0.3 (constraint + reserve + CORS).
- Attestor re-provisioned with a new constrained credential (names:[attestor2]); full loop re-tested green (endorse → mint → DNSSEC → claim wire).
- Bonus: /agents UI now detects IdPs that don't implement the agent API (e.g. sandmill.org, a human-only primary) and shows a clear 'domain doesn't support agent identities' error instead of a mid-flow failure — the one principled limitation (broker can't mint for a primary's domain: such a cert wouldn't verify at RPs) is now legible in the UI.
